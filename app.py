...
def initialize_models():
    """Initialize ML models on first use (lazy loading for deployment)"""
    global freshness_analyzer, ocr_handler
    
    if freshness_analyzer is not None and ocr_handler is not None:
        return True
    
    try:
        # Try to download model if not exists
        if not os.path.exists(MODEL_PATH):
            logger.warning("Model file not found. Attempting to download...")
            try:
                ...
                if huggingface_repo:
                    logger.info(f"Downloading model from Hugging Face: {huggingface_repo}")
                    ...
                    logger.error("No model source configured. Set HUGGINGFACE_REPO or GOOGLE_DRIVE_FILE_ID env vars")
                    return False
...
def predict():
    ...
    try:
        ...
        if not initialize_models():
            ...
            return jsonify({
                "success": False,
                "error": "ML model not initialized. Please configure HUGGINGFACE_REPO or GOOGLE_DRIVE_FILE_ID environment variables"
            }), 503
...
def upload_image():
    ...
    try:
        # Initialize models if not already done
        if not initialize_models():
            logger.error("Models not available for OCR")
            return jsonify({
                "success": False,
                "error": "ML model not initialized. Please configure HUGGINGFACE_REPO or GOOGLE_DRIVE_FILE_ID environment variables"
            }), 503
